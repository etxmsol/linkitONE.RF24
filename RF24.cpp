...
{
	char s[256];
	sprintf(s, "STATUS\t\t = 0x%02x RX_DR=%x TX_DS=%x MAX_RT=%x RX_P_NO=%x TX_FULL=%x\r\n",
           status,
...
void RF24::print_byte_register(const char* name, uint8_t reg, uint8_t qty)
{
  ...
  sprintf(s, ("%s\t%c ="),name,extra_tab);
  Serial.write(s);
  while (qty--)
  {
    sprintf(s, (" 0x%02x"),read_register(reg++));
    Serial.write(s);
  }
  Serial.write("\r\n");
}
...
void RF24::printDetails(void)
{
  ...
  char s[256];
  sprintf(s,("Data Rate\t = %s\r\n"),rf24_datarate_e_str_P[getDataRate()]);
  Serial.write(s);
  sprintf(s,("Model\t\t = %s\r\n"),rf24_model_e_str_P[isPVariant()]);
  Serial.write(s);
  sprintf(s,("CRC Length\t = %s\r\n"),rf24_crclength_e_str_P[getCRCLength()]);
  Serial.write(s);
  sprintf(s,("PA Power\t = %s\r\n"),rf24_pa_dbm_e_str_P[getPALevel()]);
  Serial.write(s);
}
...
bool RF24::available(uint8_t* pipe_num)
{
  ...
  if (result)
  {
	  Serial.write("available on pipe !!!\r\n");
